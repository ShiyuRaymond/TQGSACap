...
class YOLOWorldDualPAFPN(YOLOWorldPAFPN):
    ...
    def __init__(self,
        ...
        # self.image_pooler = MODELS.build(image_pooler)
        # self.cross_enhancer = CrossModalFeatureEnhancer(512)
    ...
    def forward(self, img_feats: List[Tensor], txt_feats: Tensor) -> tuple:
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            if self.upsample_feats_cat_first:
                ...
                top_down_layer_inputs = torch.cat([feat_low, upsample_feat], 1)
            inner_out = self.top_down_layers[len(self.in_channels) - 1 - idx](
                top_down_layer_inputs, txt_feats.expand(-1, 76, -1).transpose(0,1))
            inner_outs.insert(0, inner_out)

        # cross-enhancer
        # image_features = self.image_pooler(inner_outs)
        # txt_feats = self.cross_enhancer(txt_feats, image_features)
        # txt_feats = txt_feats.transpose(0,1)
        txt_feats = self.text_enhancer(txt_feats, inner_outs)
        # txt_feats = self.image_pooler(txt_feats, inner_outs)
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            downsample_feat = self.downsample_layers[idx](feat_low)
            out = self.bottom_up_layers[idx](torch.cat(
                [downsample_feat, feat_high], 1), txt_feats.expand(-1, 76, -1))
            outs.append(out)
        ...
        for idx in range(len(self.in_channels)):
            results.append(self.out_layers[idx](outs[idx]))
        # txt_feats = txt_feats.transpose(0, 1)
        results.append(txt_feats)
        return tuple(results)
        # return self.out_layers[0](outs[0]), self.out_layers[1](outs[1]), self.out_layers[2](outs[2]),txt_feats.transpose(0, 1)
